...
/*December 1993, modified for SLINK clipping by A. A. Schaffer*/
/*February 8, 1994, fixed two bugs in segsexup, A. A. Schaffer*/
...
/*conditional probability of first as a genotype multiplied by probability
 ...
 /*Iterate over all joint genotypes of the child*/
  ...
  for(here = 0; here < mgeno; here++) {
    ...
    tempr = newwithr[here];
    temp1 += tempr * qsumcache[here];  /*Fixed bug here 2-8-94, AAS*/
  }
...
/*initialize gene array and set up flag array*/
    for(i = 0; i < fgeno; i++)  /*Fixed bug here, 2-9-94, AAS*/
      gene[i] = 0.0;
  if(malechild[0]) {
    for(i = 0; i < mgeno; i++) {
      if(newwithr[i] == 0.0) continue;
