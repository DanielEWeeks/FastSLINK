program msim(input, output, outfile, ipedfile, datafile, speedfile, msim, lodfile, simout);
{3 July 1992}
...
const
    version = '2.51';			{VERSION OF MSIM, based on version 4.91 of LINKAGE}
    print = false;				{print = FALSE turns off most output for faster execution}
    lodprint = true;				{FALSE turns off output to the lodfile for faster execution}
    maxpnt = 27;				{Max. number of points at which the lik. can be evaluated}
    ...
    maxchild = 15;				{MAXIMUM NUMBER OF FULLSIBS IN A SIBSHIP}
    ...
    score = true;				{LEAVE score TRUE FOR msim TO WORK CORRECTLY}
    {CALCULATE LOD SCORES}
    byfamily = true;				{LEAVE byfamily TRUE FOR msim TO WORK CORRECTLY}
...
type
    ...
    hapvector = array [1..maxlocus] of 0..maxall;
...
var
    zlod: array [1..maxped + 1, 1..maxpnt] of double;
    ztheta: array [1..maxpnt] of double;
    jmax, irep: integer;
    rep, zmax: double;
    clods: array [1..maxped + 1, 1..3] of integer; {counts of lods >= constants 1,2,3}
    lodlimit: array [1..3] of double;
    nrep, ip, npt, j: integer; {nrep is a counter of number of replications}
    aver, variance, big, small: array [1..maxped + 1, 1..maxpnt] of double;
    avermaxlod, variancemaxlod, bigmaxlod, smallmaxlod: array [1..maxped + 1] of double;
    ...
    rare, risk1, risk2: array [1..maxfem] of boolean;
    riskmale: array [1..maxhap] of boolean;
    ...
    riskall: 1..maxall;
    {OTHERS}
    i, risksys, mutsys, nlocus, which, lastpriv, thissystem: integer;
    ...
    segscale, mutmale, mutfemale, like, alike, tlike, finish, inc, distratio, scorevalue, holdtheta: double;
    interfer, disequi, sexlink, risk, sexdif, readfemale, mapping, dolod, firstapprox, lasttime, firsttime, firsteff: boolean;
    outfile, ipedfile, datafile, speedfile, msim, lodfile, simout: text;
    chtemp: char;
    k: integer;


function quadmax(x1, x2, x3, f1, f2, f3: double): double;
...
var
    d, d1, d2, ex: double;
begin
    if x1 = x2 then 			{worry about x values}
	if f2 > f3 then 
	    quadmax := f2
	else 
	    quadmax := f3
	...
	d := (x2 - x1) / (x3 - x2);
	if (d > 4.0) or (d < 0.25) or (f1 < -100.0) then  {then simply look at max(fi)}
	    if (f1 >= f2) and (f1 >= f3) then 
		quadmax := f1
	    else if (f3 >= f2) and (f2 >= f1) then 
		quadmax := f3
	    else 
		quadmax := f2
	else if (f1 = f2) and (f2 = f3) then  {verify f values}
	    quadmax := f1
	else if (f2 >= f1) and (f2 >= f3) then begin
	    d := x3 - x1;
	    d1 := (f3 - f1) / d;
	    d2 := 0.5 * d / ((f2 - f1) / (x2 - x1) - (f3 - f2) / (x3 - x2));
	    ex := 0.5 * (x1 + x3) + d1 * d2;
	    quadmax := f2 + 0.5 * sqr(x2 - ex) / d2
	end else if f1 > f3 then 
	    quadmax := f1
	else 
	    quadmax := f3
    end
...
begin
	...
	if firsteff then 
		...
		writeln(output, ' Maxneed may be reduced to ', here)
end; { recombination }
...
procedure inputdata;
	...
	case nerror of
	    ...
	    41:
		writeln('Error in datafile, line 1, item 4: Program code not for MSIM/MLINK')
	end;
	...
	var
		...
		with locus^ do 
			...
			if i > maxfact then begin
			    writeln(msim, ' *** ERROR: maxfact not big enough ***');
	...
	risk := risksys <> 0;
	sexlink := autosomal = 1;
	write('YOU ARE USING LINKAGE/MSIM (V', version, ') WITH', nlocus: 3, '-POINT');
	if sexlink then 
	...
	readln(datafile, which, inc, finish);
...
begin				{inputdata}
    ...
    rewrite(msim, 'msim.dat'); {SUN}
    if lodprint then 
	rewrite(lodfile, 'lodfile.dat'); {SUN}
    readloci
end; { inputdata }
...
label
	...
	var
	    thisval: integer; { Corrected to match Version 5.1 }
			...
			for j := sstart to send do begin
				...
				for k := 1 to nchild do 
					...
					if malechild[k] then 
					    ...
					    temp[k] := temp[k] + 2.0 * temp2 * ((1 - pf) * (1 - ps) * genarray[g1] + ps * (1 - pf) * genarray[g3] + pf * (1 - ps) * genarray[g5] + pf * ps * genarray[g7]) {Ask Mark}
			    end;
...
begin
    ...
    if maletheta^.theta[which] = 0.0 then 
	firsttime := true;
    if femaletheta^.theta[which] = 0.0 then 
	firsttime := true;
...
begin
	...
	if score and not risk then 
	    if maletheta^.theta[which] = 0.5 then 
		stand[thisped] := like / log10;
	if byfamily and print then 
	...
	if byfamily then begin
	    ...
	    if score and not risk and print then 
    ...
    if lodprint then 
	if not ((nlocus = 2) and (maletheta^.theta[1] = 0.50)) then 
	    for i := 1 to nlocus - 1 do 
		write(lodfile, maletheta^.theta[i]: 6: 3);
    for locip := 1 to opeds do begin
	{Write out lodscores by pedigree on the same line as the theta(s) }
	if lodprint then 
	    if not ((nlocus = 2) and (maletheta^.theta[1] = 0.50)) then 
		write(lodfile, lods[locip]: 12: 6);
	{ Store the lod scores for the point npt and the pedigree locip
	in the array zlod }
	zlod[locip, npt] := lods[locip];
	if nrep = 1 then begin
	    aver[locip, npt] := lods[locip];
	    variance[locip, npt] := 0.0;
	    small[locip, npt] := lods[locip];
	    big[locip, npt] := lods[locip]
	end else begin
	    rep := nrep;
	    { rep := double(nrep);}
	    aver[locip, npt] := (rep - 1.0) * aver[locip, npt] / rep + lods[locip] / rep;
	    variance[locip, npt] := (rep - 1.0) * variance[locip, npt] / rep + sqr(lods[locip] - aver[locip, npt]) / (rep - 1.0);
	    small[locip, npt] := min(small[locip, npt], lods[locip]);
	    big[locip, npt] := max(big[locip, npt], lods[locip])
	end
    end; {loop on locip}
    ...
    for locip := 2 to opeds do 
	lods[1] := lods[1] + lods[locip];
    locip := opeds + 1;
    zlod[locip, npt] := lods[1];
    if nrep = 1 then begin
	aver[locip, npt] := lods[1];
	variance[locip, npt] := 0.0;
	small[locip, npt] := lods[1];
	big[locip, npt] := lods[1]
    end else begin
	rep := nrep;
	{rep := double(nrep);}
	aver[locip, npt] := (rep - 1.0) * aver[locip, npt] / rep + lods[1] / rep;
	variance[locip, npt] := (rep - 1.0) * variance[locip, npt] / rep + sqr(lods[1] - aver[locip, npt]) / (rep - 1.0);
	small[locip, npt] := min(small[locip, npt], lods[1]);
	big[locip, npt] := max(big[locip, npt], lods[1])
    end;
...
begin
    writeln(output);
    writeln(output, 'Program MSIM version ', version);
    writeln(output);
...
{ assign(speedfile,'speedfil.dat');
  ...
  assign(simout,'simout.dat'); }
	...
	write(output, '*');
    writeln(output);
    writeln(output, ' This is MSIM, which is a modified version of MLINK');
    ...
    writeln(output, ' The statistical summary is written to msim.dat');
    for i := 1 to 66 do 
	write(output, '*');
    writeln(output);
...
procedure quadout(var fout: text);
var
    i, ip: integer;
begin
    writeln(fout, ' Average Maximum Lod Scores based on quadratic interpolation ');
    for i := 1 to 66 do 
	write(fout, '-');
    writeln(fout);
    writeln(fout, 'Pedigree  Average        StdDev         Min            Max');
    for ip := 1 to opeds + 1 do 
	if ip <= opeds then 
	    writeln(fout, ip: 6, avermaxlod[ip]: 15: 6, sqrt(variancemaxlod[ip]): 15: 6, smallmaxlod[ip]: 15: 6, bigmaxlod[ip]: 15: 6)
	else begin
	    writeln(fout);
	    writeln(fout, 'Study ', avermaxlod[ip]: 15: 6, sqrt(variancemaxlod[ip]): 15: 6, smallmaxlod[ip]: 15: 6, bigmaxlod[ip]: 15: 6);
	    for i := 1 to 66 do 
		write(fout, '-');
	    writeln(fout)
	end;
	...
	write(fout, '-');
    writeln(fout);
    writeln(fout, '   Number of (interpolated) maximum lod scores greater than');
    writeln(fout, '   a given constant');
    for i := 1 to 66 do 
	write(fout, '-');
    writeln(fout);
    writeln(fout, ' Constant  Pedigree  Number  Percent');
    for i := 1 to 3 do 
	for ip := 1 to opeds + 1 do 
	    if ip <= opeds then 
		writeln(fout, lodlimit[i]: 8: 3, ip: 10, clods[ip, i]: 8, 100.0 * clods[ip, i] / nrep: 9: 3)
	    else 
		writeln(fout, lodlimit[i]: 8: 3, 'Study': 10, clods[ip, i]: 8, 100.0 * clods[ip, i] / nrep: 9: 3);
    for i := 1 to 66 do 
	write(fout, '-');
    writeln(fout)
end; { quadout } {quadout}
...
begin				{msim}
    ...
    for k := minint to maxcensor do 
	censorstruct^.censor[k] := false;
    new(gennustruct);
    {  gennustruct:=gennuptr(NewPtr(SizeOf(gennurec)));}
    firstapprox := true;
    getlocations;

    write(outfile, 'LINKAGE/MSIM (V', version, ') WITH', nlocus: 3, '-POINT');
    if sexlink then 
	...
	if nrep mod 50 = 0 then 
	    writeln(output, ' ', nrep: 1, ' replicates done');
	firsttime := true;
	readpedseg;
	for i := 1 to totperson do 
	    person[i]^.store := nil;
	...
	if score and not risk then begin
	    ...
	    ztheta[npt] := maletheta^.theta[which];
	    iterpeds;
	...
	while maletheta^.theta[which] <= finish do begin
	    iterpeds;
	    { Store thetas in ztheta array for quadratic interpolation }
	    ztheta[npt] := maletheta^.theta[which];
	    maletheta^.theta[which] := maletheta^.theta[which] + inc;
	    npt := npt + 1;
		...
		writeln(output, ' Either increase the constant maxpnt and re-compile');
		writeln(output, ' or reduce the number of thetas by changing the datafile.dat'); 
		writeln(output, 'Press return to halt...');
		readln(input);
		halt; {SUN}
	    end
	end; { increment on theta[which] }
	...
	for i := 1 to totperson do  {all persons in this replicate}
	...
	if not sexdif and (nlocus = 2) and (npt > 2) then 
	    for i := 1 to opeds + 1 do begin
		zmax := -100000.0;
		for ip := 1 to npt do 
		    if zmax <= zlod[i, ip] then begin
			jmax := ip;
			zmax := zlod[i, ip]
		    end;
		if jmax = 1 then 		{Maximum at ztheta=0.50}
		    zmax := 0.0
		else begin
		    if jmax = 2 then 
			jmax := 3;	{jmax=2 represents smallest theta}
		    if jmax = npt then 
			jmax := npt - 1; {jmax=npt represent largest theta}
		    zmax := quadmax(ztheta[jmax - 1], ztheta[jmax], ztheta[jmax + 1], zlod[i, jmax - 1], zlod[i, jmax], zlod[i, jmax + 1])
		end;

		{ Update counts of maximum lod scores greater than 1, 2 or 3 }
		if zmax >= lodlimit[1] then 
		    clods[i, 1] := clods[i, 1] + 1;
		if zmax >= lodlimit[2] then 
		    clods[i, 2] := clods[i, 2] + 1;
		if zmax >= lodlimit[3] then 
		    clods[i, 3] := clods[i, 3] + 1;
		...
		if nrep = 1 then begin
		    avermaxlod[i] := zmax;
		    variancemaxlod[i] := 0.0;
		    smallmaxlod[i] := zmax;
		    bigmaxlod[i] := zmax
		end else begin
		    rep := nrep;
		    { rep := double(nrep);}
		    avermaxlod[i] := (rep - 1.0) * avermaxlod[i] / rep + zmax / rep;
		    variancemaxlod[i] := (rep - 1.0) * variancemaxlod[i] / rep + sqr(zmax - avermaxlod[i]) / (rep - 1.0);
		    smallmaxlod[i] := min(smallmaxlod[i], zmax);
		    bigmaxlod[i] := max(bigmaxlod[i], zmax)
		end
	    end
    end {for i  begin};				{ of while not eof(ipedfile) }
    close(ipedfile); {SUN}
    close(speedfile); {SUN}
    close(datafile); {SUN}
    close(outfile); {SUN}
    { Copy most recent simout.dat to msim.dat }
    writeln(msim, ' ********* Data from most recent SIMOUT.DAT *********');
{assign(simout,'simout.dat');}
    reset(simout, 'simout.dat'); {SUN}
    ...
    writeln(msim, ' ********* End of most recent SIMOUT.DAT *********');
    writeln(msim);
    close(simout); {SUN}
    { Output actual order to 'msim.dat' }
    write(msim, ' ORDER OF LOCI: ');
    for i := 1 to nlocus do begin
	thissystem := 1;
	while order[thissystem] <> i do 
	    thissystem := thissystem + 1;
	write(msim, thissystem: 3)
    end;
    writeln(msim);
    writeln(output);
    if nlocus = 2 then 
	writeln(msim, ' Average Lod Scores at Given Thetas')
    else 
	writeln(msim, ' Average Multipoint Lod Scores at Given Thetas');
    writeln(msim);
    if nlocus = 2 then 
    ...
    writeln(msim, 'Number of replicates = ', nrep: 6);
    writeln(output);
    writeln(msim);
    for i := 1 to 66 do 
	write(msim, '-');
	...
	if sexdif then 
	    write(msim, 'MALE THETAS   ')
	else 
	    write(msim, 'THETAS ');
	for i := 1 to nlocus - 1 do 
	    write(msim, maletheta^.theta[i]: 6: 3);
	if not sexdif and (nlocus = 2) then 
	    if interfer then 
		write(msim, maletheta^.theta[nlocus]: 6: 3);
	writeln(msim);
	if sexdif then begin
	    write(msim, 'FEMALE THETAS ');
	    for i := 1 to nlocus - 1 do 
		write(msim, femaletheta^.theta[i]: 6: 3);
	    if interfer then 
		write(msim, femaletheta^.theta[nlocus]: 6: 3);
	    writeln(msim)
	end;
	for i := 1 to 66 do 
	    write(msim, '-');
	...
	writeln(msim, 'Pedigree  Average        StdDev         Min            Max');
	for ip := 1 to opeds + 1 do begin
	    if ip <= opeds then begin
		writeln(output, ip: 6, aver[ip, npt]: 15: 6, sqrt(variance[ip, npt]): 15: 6, small[ip, npt]: 15: 6, big[ip, npt]: 15: 6);
		writeln(msim, ip: 6, aver[ip, npt]: 15: 6, sqrt(variance[ip, npt]): 15: 6, small[ip, npt]: 15: 6, big[ip, npt]: 15: 6)
	    end else begin
		writeln(msim);
		writeln(output);
		writeln(output, 'Study ', aver[ip, npt]: 15: 6, sqrt(variance[ip, npt]): 15: 6, small[ip, npt]: 15: 6, big[ip, npt]: 15: 6);
		writeln(msim, 'Study ', aver[ip, npt]: 15: 6, sqrt(variance[ip, npt]): 15: 6, small[ip, npt]: 15: 6, big[ip, npt]: 15: 6);
		for i := 1 to 66 do 
		    write(msim, '-');
		writeln(msim);
		for i := 1 to 66 do 
    ...
    if not sexdif and (nlocus = 2) and (npt > 2) then begin
	quadout(msim);
	quadout(output)
    end;
    acknowl(msim)
